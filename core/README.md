...
| `orchestrations/` | Modelo y guardado implementados con tests: una fila por orquestación en Postgres con la guarda de cliente, y el grafo validado como candado —conexiones que apuntan a nodos inexistentes y ciclos se rechazan al guardar, porque el motor los descartaba en silencio. Faltan el motor de ejecución, la programación por cron y la interfaz. **Sin verificar contra un tenant real todavía** |
